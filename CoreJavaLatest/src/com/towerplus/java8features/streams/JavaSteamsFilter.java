...
public class JavaSteamsFilter {
	...
	public static void main(String[] args) {
		...
		System.out.println("Applying map by appending String  is " + filteredList2);
		
		// Using Map function
		List<Integer> numbers = Arrays.asList(9, 10, 3, 4, 7, 3, 4);
		List<Integer> distinctNumbers = numbers.stream().map( i -> i*i).distinct().collect(Collectors.toList());
		System.out.println("Map multiplied Number using map is " + distinctNumbers);
		
		//Number statistics
		List<Integer> numbers2 = Arrays.asList(9, 10, 3, 4, 7, 3, 4);
		System.out.println("Number  mapToInt Result is " + numbers2.stream().mapToInt(x -> x).summaryStatistics()); 
		...
		System.out.println("using map  functions are ");
